...
from pogo_iphone_renamer.game_navigation import (
...
class ResilientLandscapeNavigationTests(unittest.TestCase):
    ...
    def test_fresh_detail_pixels_override_stale_inventory_accessibility(self) -> None:
        ...
        with patch(
            "pogo_iphone_renamer.game_navigation.snapshot_is_black",
            return_value=False,
        ), patch(
            "pogo_iphone_renamer.game_navigation._fresh_screenshot_text",
            return_value=("CP 100 20 / 20 HP 1.0 kg", True),
        ), patch(
            "pogo_iphone_renamer.game_navigation.base.local_page_state",
            return_value="DETAIL",
        ):
    ...
    def test_entry_navigation_closes_all_pokedex_layers_before_continuing(self) -> None:
        ...
        proxy = object()
        with patch(
            "pogo_iphone_renamer.game_navigation._wait_until_visible",
            return_value=pokedex,
        ), patch(
            "pogo_iphone_renamer.game_navigation._close_detail_record_overlay_if_needed",
            return_value=(pokedex, False),
        ), patch(
            "pogo_iphone_renamer.game_navigation.robust_page_state",
            side_effect=[
                "POKEDEX_DETAIL",
                "POKEDEX_REGION_GRID",
                "POKEDEX_INDEX",
            ...
            "pogo_iphone_renamer.game_navigation.base._tap"
        ) as tap, patch(
            "pogo_iphone_renamer.game_navigation.base._ensure_stage_geometry_for_state",
            side_effect=lambda _proxy, snapshot, _state, **_kwargs: snapshot,
        ), patch(
            "pogo_iphone_renamer.game_navigation.base._next_snapshot",
            side_effect=[grid, index, detail],
        ), patch(
            "pogo_iphone_renamer.game_navigation._ORIGINAL_NAVIGATE",
            return_value=(detail, measurement),
        ):
            returned = navigate_to_appraisal(proxy, pokedex)
    ...
    def test_map_transition_returns_when_inventory_is_already_reached(self) -> None:
        ...
        with patch(
            ...
            "pogo_iphone_renamer.game_navigation._wait_for_target",
    ...
    @patch("pogo_iphone_renamer.game_navigation.ocr_mcp_screenshot")
