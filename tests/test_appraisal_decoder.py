...
from pogo_iphone_renamer.appraisal_decoder import (
    _is_any_iv_fill,
    _row_consensus_endpoint,
    measure_decoded_appraisal,
    measure_upright_appraisal,
)
...
class AppraisalDecoderTests(unittest.TestCase):
            ...
            for erode in (0, 1, 2):
                for value in range(16):
                    with self.subTest(offset=offset, erode=erode, value=value):
                        result = measure_decoded_appraisal(
    ...
    def test_stage_manager_crop_does_not_add_one_to_each_iv(self) -> None:
        ...
        result = measure_upright_appraisal(image)
        self.assertEqual(
            (result.attack, result.defense, result.stamina),
            values,
        )
