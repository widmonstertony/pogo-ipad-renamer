...
from .gui_ipad_landscape_v4 import IPadLandscapeRenamerAppV4
from .gui_native import collect_native_status


class IPadLandscapeRenamerAppV5(IPadLandscapeRenamerAppV4):
    def _build_ui(self) -> None:
        super()._build_ui()
        self._append_log("黑帧提示已区分：锁屏、MCP 截图故障与真实游戏页面不会混为一谈。")
    ...
    def _run_worker(self, write_enabled: bool) -> None:
        ...
        command = [
            ...
            "-u",
            "-m",
            "pogo_iphone_renamer.ipad_landscape_agent_v15",
            "--mode",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Pokémon GO iPad 横屏整理助手 v15")
    ...
    window = tk.Tk()
    IPadLandscapeRenamerAppV5(window, root)
    window.mainloop()
    return 0
