...
from .gui_native import (
    NativeRenamerApp,
    collect_native_status,
    friendly_native_event,
)
from .gui_hdpi import enable_per_monitor_dpi, project_root, self_check


class ResilientNativeRenamerApp(NativeRenamerApp):
    def _build_ui(self) -> None:
        super()._build_ui()
        self._append_log("MCP 屏幕读取超时：120 秒；断线后只自动重试读取，绝不重试写操作。")
    ...
    def _run_worker(self, write_enabled: bool) -> None:
        ...
        command = [
            ...
            "-u",
            "-m",
            "pogo_iphone_renamer.native_agent_v2",
            "--mode",
...
def main(argv: list[str] | None = None) -> int:
    ...
    window = tk.Tk()
    ResilientNativeRenamerApp(window, root)
    window.mainloop()
    return 0
