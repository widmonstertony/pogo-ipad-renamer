...
from .gui_ipad_landscape_v2 import IPadLandscapeRenamerAppV2
from .gui_native import collect_native_status


class IPadLandscapeRenamerAppV3(IPadLandscapeRenamerAppV2):
    def _build_ui(self) -> None:
        super()._build_ui()
        self._append_log("提交成功判定：提交前逐字一致 + OK 成功 + 弹窗消失 + 返回详情页。")
        self._append_log("已带 IV 数字标记的昵称会自动保留并跳过。")
    ...
    def _run_worker(self, write_enabled: bool) -> None:
        ...
        command = [
            ...
            "-u",
            "-m",
            "pogo_iphone_renamer.ipad_landscape_agent_v13",
            "--mode",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Pokémon GO iPad 横屏整理助手 v13")
    ...
    window = tk.Tk()
    IPadLandscapeRenamerAppV3(window, root)
    window.mainloop()
    return 0
