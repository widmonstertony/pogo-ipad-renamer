...
def refresh_game_foreground_capture(proxy: SafeProxy) -> Snapshot:
    ...
    if navigation.snapshot_is_black(home):
        emit(
            "status",
            message=(
...
def wait_for_capture_channel(
    ...
    *,
    ...
    if not navigation.snapshot_is_black(snapshot):
        return snapshot
...
def run(mode: str, settings: Settings, ollama_url: str, model: str) -> int:
    previous = navigation._wait_until_visible
    navigation._wait_until_visible = wait_for_capture_channel
    try:
        return navigation.run(mode, settings, ollama_url, model)
    finally:
        navigation._wait_until_visible = previous


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="iPad landscape truthful capture renamer")
