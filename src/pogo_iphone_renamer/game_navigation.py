...
def run(mode: str, settings: Settings, ollama_url: str, model: str) -> int:
    ...
    base.navigate_to_appraisal = navigate_to_appraisal
    try:
        return transition.run(mode, settings, ollama_url, model)
    finally:
        base.navigate_to_appraisal = previous


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="iPad landscape resilient Pokémon GO renamer")
