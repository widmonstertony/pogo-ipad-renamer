...
from .rename_observation import _mark_rename_observation
from .accessibility_names import exact_name_field
from .name_input import _backspace_current_name
...
from .rename_controls import tap_ok
    ...
    for attempt in range(_SUBMIT_TAP_LIMIT):
        ...
        if not use_accessibility or not _tap_accessibility_ok(proxy):
            try:
                ...
                if transition.image and navigation.robust_page_state(transition) == "DETAIL":
                    emit(
                        "status",
                        message=(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Pure-Python locked iPad renamer with verified keyboard dismissal"
    )
