...
from .accessibility_names import exact_name_field
from .device_recovery import (
    DeviceLockRecoveryRequired,
    wait_for_capture_channel,
    wait_for_unlocked_snapshot,
)
from .rename_submission import (
...
from .field_verification import _verified_entered_value
from .appraisal_retry import (
    AppraisalMeasurementUnavailable,
    _navigate_with_read_only_measurement_retry,
)
from .stale_dialog_recovery import _navigate_with_complete_stale_recovery
from .landscape_cv import is_fullscreen_portrait_game_frame, rotate_mcp_image_upright
from .appraisal_cv import measure_appraisal
...
from .text_localization import locate_exact_text_from_mcp
from .name_recognition import (
    HP_LINE,
    NUMBER_TOKEN,
    NameRegionResult,
    analyze_name_region,
...
base.measure_ipad14_6_appraisal = measure_appraisal
...
def _navigate_from_current_detail_only(
    ...
    The historical navigation appraise adapter can start from MAP, MAIN_MENU, or
    ...
    The adapter normally needs no navigation transition when it begins at DETAIL: it
    only opens the detail menu, chooses Appraise, and (where required) advances
    the appraisal dialogue once.  Do not call the stale-dialog navigation wrapper here: that
    ...
    The appraisal retry reader wraps its import-time reference to the base navigator.  It
...
def _close_appraisal(proxy: SafeProxy) -> Snapshot:
    for attempt in range(2):
        ...
        for observation in range(5):
            ...
            if navigation.snapshot_is_black(candidate):
...
def _ensure_plain_detail(proxy: SafeProxy, snapshot: Snapshot) -> Snapshot:
    ...
    state = navigation.robust_page_state(snapshot)
    ...
    while state != "DETAIL":
        ...
        snapshot, state = navigation._transition(proxy, snapshot, state)
    base._validate_expected("DETAIL", snapshot)
    return snapshot
...
def _select_pokemon_tab_from_egg_overview(proxy: SafeProxy, snapshot: Snapshot) -> Snapshot:
    ...
    for attempt, (mapping, touch_x, touch_y) in enumerate(candidates, start=1):
        ...
        for read_attempt in range(3):
            ...
            if (
                navigation.robust_page_state(candidate) == "INVENTORY"
...
def _open_next_inventory_card_after_pager_exit(
    ...
    if navigation.robust_page_state(inventory) != "INVENTORY":
        raise PolicyViolation(
    ...
    for scroll_attempt in range(2):
        ...
        if current_index is not None and current_index + 1 < len(cards):
            ...
            for read_attempt in range(6):
                if navigation.robust_page_state(candidate) == "DETAIL":
                    try:
...
def _return_detail_to_pager_resume_inventory(
    ...
    if navigation.robust_page_state(snapshot) != "DETAIL":
...
def _confirm_low_confidence_measurement(proxy: SafeProxy, snapshot: Snapshot, measurement):
    ...
    The calibrated appraisal reader already requires divider geometry, physical track start,
...
def _process_one(
    ...
    *,
    ...
    try:
        # Reuse the batch close helper instead of the rename dialog helper's historical one-frame
...
def run(mode: str, settings: Settings) -> int:
    with DeviceRunLock(settings.journal_path.parent / "iphone-mcp.lock"):
        ...
        previous_wait_until_visible = navigation._wait_until_visible
        ...
        navigation._wait_until_visible = _wait_without_game_restart
        try:
            ...
            resume_state = navigation.robust_page_state(snapshot)
            ...
            navigation._wait_until_visible = previous_wait_until_visible
            base.ORIENTATION = previous_orientation


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Safe batch iPad Pokémon renamer")
