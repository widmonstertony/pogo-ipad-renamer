...
from .gui_native_v4 import DeterministicReadonlyApp
from .gui_hdpi import enable_per_monitor_dpi, project_root, self_check


class SplitVisionReadonlyApp(DeterministicReadonlyApp):
    def _build_ui(self) -> None:
        super()._build_ui()
        self._append_log("视觉输入已分流：OCR/界面文本与原始截图分别传给本地模型。")
    ...
    def _run_worker(self, write_enabled: bool) -> None:
        ...
        command = [
            ...
            "-u",
            "-m",
            "pogo_iphone_renamer.native_agent_v5",
            "--mode",
        ...
        try:
            ...
            self.events.put(
                (
                    "log",
                    "本地执行进程已启动。"
                    + ("改名权限仅对本次运行开启。" if write_enabled else "当前为确定性严格只读流程。"),
                )
            )
            ...
            self.events.put(("fatal", f"无法启动本地执行进程：{exc}"))
        finally:
            self.process = None
...
def main(argv: list[str] | None = None) -> int:
    ...
    window = tk.Tk()
    SplitVisionReadonlyApp(window, root)
    window.mainloop()
    return 0
