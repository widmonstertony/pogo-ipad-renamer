...
from .gui_ipad_landscape_v3 import IPadLandscapeRenamerAppV3
from .gui_native import collect_native_status


class IPadLandscapeRenamerAppV4(IPadLandscapeRenamerAppV3):
    def _build_ui(self) -> None:
        super()._build_ui()
        self._append_log("入口导航采用双通道验证与加载等待；黑屏不会被当成地图。")
        self._append_log("只有确认原页面完全未变化时，导航点击才会有界重试一次。")
    ...
    def _run_worker(self, write_enabled: bool) -> None:
        ...
        command = [
            ...
            "-u",
            "-m",
            "pogo_iphone_renamer.ipad_landscape_agent_v14",
            "--mode",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Pokémon GO iPad 横屏整理助手 v14")
    ...
    window = tk.Tk()
    IPadLandscapeRenamerAppV4(window, root)
    window.mainloop()
    return 0
