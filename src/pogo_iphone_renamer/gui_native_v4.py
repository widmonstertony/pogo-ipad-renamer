...
from .gui_native_v3 import QwenSafeRenamerApp
from .gui_hdpi import enable_per_monitor_dpi, project_root, self_check


class DeterministicReadonlyApp(QwenSafeRenamerApp):
    def _build_ui(self) -> None:
        super()._build_ui()
        self._append_log("只读预演采用固定流程：读取前台 + 读取屏幕 + 一次模型报告，不会循环观察。")
    ...
    def _run_worker(self, write_enabled: bool) -> None:
        ...
        command = [
            ...
            "-u",
            "-m",
            "pogo_iphone_renamer.native_agent_v4",
            "--mode",
...
def main(argv: list[str] | None = None) -> int:
    ...
    window = tk.Tk()
    DeterministicReadonlyApp(window, root)
    window.mainloop()
    return 0
