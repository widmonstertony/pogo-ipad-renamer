...
def _exact_nickname_visible(proxy: SafeProxy, nickname: str) -> bool:
    if proxy.observation is not None:
        try:
            if names.exact_name_field(Snapshot(proxy.observation.text, None)) == nickname:
                return True
