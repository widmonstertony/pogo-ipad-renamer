...
def local_page_state(snapshot: Snapshot) -> str:
    ...
    if snapshot.image and ORIENTATION in {
        ...
        # game crop so the base navigation cannot contradict the enhanced navigation layer
...
def _tap(proxy: SafeProxy, key: str) -> None:
    ...
    if key == "DETAIL_MENU" and ORIENTATION in {
        ...
        from .rename_controls import tap_ocr_control
...
def navigate_to_appraisal(proxy: SafeProxy, snapshot: Snapshot) -> tuple[Snapshot, Any]:
    ...
    if not snapshot.image:
        ...
        # evidence that navigation failed: callers with the appraisal retry adapter catch
    ...
    # Propagate the read failure so the appraisal retry adapter can wait for fresh frames
