...
from . import verified_rename as rename_verification
from . import device_recovery as device_recovery
...
def _verified_dialog_snapshot(
    ...
    image = rename_verification._screenshot_only(proxy)
...
def run(mode: str, settings: Settings, ollama_url: str = "", model: str = "") -> int:
    previous = rename_verification._open_verified_rename_dialog
    rename_verification._open_verified_rename_dialog = _open_verified_rename_dialog
    try:
        return device_recovery.run(mode, settings, ollama_url, model)
    finally:
        rename_verification._open_verified_rename_dialog = previous


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Pure-Python iPad landscape deterministic renamer"
    )
