...
def main() -> int:
    ...
    proxy.call_tool(
        "launch_app",
        {
            "bundle_id": settings.pokemon_go_bundle_id,
            "_observation_token": proxy.observation.token,
            "_intent": "launch configured Pokemon GO foreground after window layout probe",
            "_expected_after": "configured Pokemon GO foreground",
        },
    )
    after = screen_snapshot(proxy)
    if after.image:
        output = Path(__file__).resolve().parents[1] / ".pogo-data" / "pokemon-foreground-v44.png"
