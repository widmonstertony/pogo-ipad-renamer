...
def main() -> int:
    ...
    proxy.call_tool(
        "tap_screen",
        {
            ...
            "_intent": "navigate calibrated Stage Manager Pokemon GO main menu",
            "_expected_after": "Pokemon GO MAIN_MENU",
        },
    )
    after = screen_snapshot(proxy)
    if after.image:
        output = Path(__file__).resolve().parents[1] / ".pogo-data" / "stage-manager-after-map-v39.png"
        output.parent.mkdir(parents=True, exist_ok=True)
        Image.open(io.BytesIO(base64.b64decode(after.image))).save(output)
        print(output)
    print(f"before={robust_page_state(before)} after={robust_page_state(after)}")
    return 0
