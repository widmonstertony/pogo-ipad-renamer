...
def main() -> int:
    ...
    proxy.call_tool(
        "tap_screen",
        {
            ...
            "_intent": "navigate Pokemon GO Stage Manager window menu",
            "_expected_after": "iPad window layout menu for Pokemon GO",
        },
    )
    after = screen_snapshot(proxy)
    if after.image:
        output = Path(__file__).resolve().parents[1] / ".pogo-data" / "stage-manager-window-menu-v42.png"
