...
def main() -> int:
    ...
    proxy.call_tool(
        "tap_screen",
        {
            ...
            "_intent": "navigate calibrated Stage Manager Pokemon storage",
            "_expected_after": "Pokemon GO INVENTORY",
        },
    )
    after = screen_snapshot(proxy)
    if after.image:
        output = Path(__file__).resolve().parents[1] / ".pogo-data" / "stage-manager-after-storage-v40.png"
